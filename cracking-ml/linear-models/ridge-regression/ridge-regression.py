...
def mse_ridge_forward(y_pred: Value, y_true: Value, W: Value, alpha: np.float64):
    """
    MSE with Ridge Loss Function \n
    L = (1/n) * sum((y_pred - y_true)^2) + alpha * sum(W^2)
    """
...
def train(X_tr, y_tr, alpha, epochs=300, lr=0.01):
    ...
    for epoch in range(epochs):
        ...
        loss, diff = mse_ridge_forward(y_pred, y, W, alpha)
        ...
        # Chain rule
        # Step 1: dL/dy_pred
        y_pred.grad, dW_reg = mse_ridge_backward(y_pred, diff, X_tr.shape[0], alpha, W)
...
alphas = np.logspace(-12, 2, num=11)  # 1e-8 to 1e2
...
# Ridge (alpha=0.1)
W_ridge, b_ridge = train(X_train, y_train, alpha=best_alpha)
val_loss_ridge = val_mse(W_ridge, b_ridge, X_val, y_val)
...
print(
    f"Ridge  -> W={W_ridge.data.flatten()}, b={b_ridge.data.flatten()}, val_loss={val_loss_ridge:.4f}"
)
print(
